...
src/
 ...
 shared/        # общее для всех фич:
                #   types, lib (format, storage, errors, audit, supabase, toast,
                #           database.types, session),
                #   hooks (useRole + RoleContext, usePageTitle),
 ...
 companies/     #   api, избранное, плитки протоколов (protocols.ts, ProtocolTile,
                #   protocol-icons), разбор групп (groups.ts), паспорт завода
                #   (CompanyPassport), карточки подключений (ConnectionCard,
                #   ConnectionDetails, ChainCard), формы (CompanyForm,
                #   ConnectionForm), FilesLinksSection, HistorySection
 duty/          #   календарь (calendar.ts), api дежурств и отпусков, экспорт месяца,
                #   общая логика ячеек (dutyCell.ts, CellParts.tsx), MonthGrid,
                #   MonthAgenda, DutySummary, PeopleManager, VacationsManager,
                #   OnDutyNow
 ...
 App.tsx        # роутинг, проверка сессии (shared/lib/session), лимит 6 часов,
                #   разделение по ролям; роль раздаётся через RoleContext
